...
export default function RoundTable({ rounds, players }) {
  ...
  return (
    ...
    <tbody className="bg-white dark:bg-slate-800">
      {rounds.map((round) => (
          <tr key={round.id}>
            <td className="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">Round {round.id}</td>
